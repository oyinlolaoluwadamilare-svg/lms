...
`0008_activities` was applied to the hosted project the same way, in the same session it was
written - `schema_migrations` there now lists `0001` through `0008`. No real-project integration
test exists for it yet (M3.2's `logActivity` is what a real signed-in-session integration spec
would exercise); `tests/rls/activities.spec.ts` verifies it against local Postgres only, matching
this table's own migration-only scope.
