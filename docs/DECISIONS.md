...
| D-20 | Two related M6.7 decisions. (a) Stage regression report uses the same own/team/practice/tenant scope Pipeline metrics (M6.1), Engagement analytics (M6.5) and Task analytics (M6.6) use, gated on `analytics.view_own` — a bde is never denied, only narrowed to their own deals (the same `resolveScopedActiveDeals` resolver M6.5 already built, reused rather than re-derived). (b) The report shows the actual list of regressed active deals (deal, current stage, from/to stage of the most recent regression, when it happened), sorted most-recent-first — not the headline rate alone. | Neither `docs/04-metric-definitions.md`'s "Stage regression rate" bullet nor `docs/02-permission-matrix.md`'s footnotes state a scope, the same genuine gap D-19 already found for Task analytics. (a) Asked directly: the product owner chose the inclusive four-way model over the practice/tenant-only model M6.2-M6.4/M5.4 use. (b) This milestone's own backlog line says "stage regression **report**," the same word Loss reasons' own breakdown-style panel uses, not just "rate" — asked directly, the product owner confirmed the doc's own framing ("a leading loss indicator") is only actionable with the underlying deals visible, not a bare percentage. Reconstructed `stage_events` rows (`is_reconstructed = true`) are excluded from the regressed-deal count, extending D-16's own reasoning (a count-based "has this event" metric, not a duration-based one, but the same "migration artefact, not real activity" logic applies) rather than a fresh decision — currently inert either way, since no code path sets `is_reconstructed = true`. | Product owner (foluso.aribisala@workforcegroup.com) | 2026-09-09 |
